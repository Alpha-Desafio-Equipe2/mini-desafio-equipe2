...
export const HomePage = async (): Promise<HTMLElement> => {
  ...
  const loadProducts = async (search = "") => {
    ...
    try {
      // Use MedicineService instead of direct API
      // Note: The search parameter is not natively supported by MedicineService.getAll yet
      // We will filter client-side for now or update filtered logic if backend supports it
